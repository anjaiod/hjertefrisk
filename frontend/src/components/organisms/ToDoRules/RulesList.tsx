...
export default function RulesList({
  ...
  const getCategoryName = (id: number) => categories.find(c => c.categoryId === id)?.name || 'Unknown';
        ...
        >
          {editingId === rule.id ? (
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs font-medium text-gray-600">TODO Text</label>
                <textarea
                  value={editData.toDoText || ''}
                  onChange={(e) => setEditData({ ...editData, toDoText: e.target.value })}
            ...
            <>
              <div className="mb-2">
                <div className="text-sm font-medium text-gray-900">{rule.toDoText}</div>
